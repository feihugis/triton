...
def matmul_kernel(
    ...
    z = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_N), dtype=tl.float16)
    ...
    for k in range(0, K, BLOCK_SIZE_K):
        ...
        z += tl.dot(a, b, out_dtype=tl.float16)
...
def matmul(a, b, a_order, b_order):
    ...
    a = triton.reinterpret(a, dtype=tl.float8e4nv)
    b = triton.reinterpret(b, dtype=tl.float8e4nv)
...
problem_list = [
    ...
    [1024, 1024, 1024, False, True],
    [2048, 2048, 2048, False, True],
    [4096, 4096, 4096, False, True],
    [8192, 8192, 8192, False, True],
]
...
def test_matmul():
    for case in problem_list:
        ...
        a8 = a.to(torch.float8_e5m2)
        b8 = b.to(torch.float8_e5m2)
        z = matmul(a8, b8, a_order, b_order)

        a16 = a8.to(torch.float16)
        b16 = b8.to(torch.float16)
        golden = torch.matmul(a16, b16)
...
def benchmark(M, N, K, TRANS_A, TRANS_B, provider):
    ...
    #         lambda: torch.matmul(a16, b16), rep=100, quantiles=quantiles, fast_flush=False)
    if provider == 'triton':
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: matmul(a8, b8, a_order, b_order), rep=100, quantiles=quantiles, fast_flush=False)
